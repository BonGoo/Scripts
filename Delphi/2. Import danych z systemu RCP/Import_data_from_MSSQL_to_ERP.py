...
def synchronize_databases():
    ...
    try:
        ...
        mssql_statement = "SELECT be.USER_ID, be.CHECK_TIME , be.CHECK_TYPE, bu.EXT_BADGE, CONVERT(DATE,be.CHECK_TIME) AS DATA, CONVERT(TIME,be.CHECK_TIME) AS CZAS, DATEPART(HOUR,be.CHECK_TIME) AS CZAS_GODZINY, ROUND((CAST(DATEPART(MINUTE, be.CHECK_TIME) AS FLOAT)/60), 4) AS CZAS_MINUTY, DATEPART(HOUR,be.CHECK_TIME) + ROUND((CAST(DATEPART(MINUTE, be.CHECK_TIME) AS FLOAT)/60), 4) AS CZAS_SUMA FROM [AR-RCP].dbo.BS_EVENT be LEFT JOIN [AR-RCP].dbo.BS_USER bu on bu.ID = be.USER_ID WHERE CHECK_TIME >= dateadd(month, -1, datefromparts(year(getdate()), month(getdate()), 1)) and CHECK_TIME < datefromparts(year(getdate()), month(getdate()), 1)"
        ...
        for mssql_record in mssql_records:
            ...
            query = "UPDATE OR INSERT INTO XXX_RCP_EVENTS (USER_ID, CHECK_TIME, CHECK_TYPE, EXT_BADGE, DATA, CZAS, CZAS_GODZINY, CZAS_MINUTY, CZAS_SUMA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) MATCHING (USER_ID, CHECK_TIME)"
        ...
        # Zaktualizuj informację o ostatniej synchronizacji
        last_sync_label.config(text="Last synchronization: {}".format(time.strftime("%Y-%m-%d %H:%M:%S")))
...
# Etykieta "Synchronizacja"
sync_label = tk.Label(window, text="Ostatnia udana synchronizacja:")
